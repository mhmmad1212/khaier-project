...
<x-dynamic-component :component="$getFieldWrapperView()" :field="$field">
    <div class="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
        ...
        <div class="mb-4 flex items-center justify-between">
            <div class="text-sm font-semibold text-gray-800">اختر الأيقونة</div>

            @if($current)
                <div class="inline-flex items-center gap-2 rounded-lg border border-primary-200 bg-primary-50 px-3 py-1.5 text-xs text-primary-700">
        ...
        <div class="grid grid-cols-4 gap-2 sm:grid-cols-5 md:grid-cols-6 xl:grid-cols-8">
            @foreach($icons as $iconKey => $iconLabel)
                <button
                    ...
                    title="{{ $iconLabel }}"
                    class="group flex h-14 w-full items-center justify-center rounded-xl border transition duration-150
                    {{ $current === $iconKey
                        ? 'border-primary-600 bg-primary-50 text-primary-600 ring-2 ring-primary-200'
                        : 'border-gray-200 bg-gray-50 text-gray-700 hover:border-primary-300 hover:bg-primary-50 hover:text-primary-600' }}"
                >
