...
<div style="max-width:1200px; margin:40px auto; padding:0 16px; direction:rtl; text-align:right;">
    ...
    <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(320px, 1fr)); gap:24px;">
        @forelse($beneficiaryServices as $beneficiary_service)
            <div style="background:#fff; border:1px solid #e5e7eb; border-radius:22px; padding:24px; box-shadow:0 12px 28px rgba(15,23,42,.06); display:flex; flex-direction:column; gap:18px;">
                <div style="display:flex; align-items:center; gap:14px;">
                    ...
                    <div>
                        <h3 style="margin:0; font-size:22px; font-weight:800; color:#111827;">
                            {{ $beneficiary_service->name }}
                        </h3>
                    </div>
                </div>

                @if(!empty($beneficiary_service->conditions))
                ...
                <div style="display:flex; flex-wrap:wrap; gap:10px; margin-top:auto;">
                    @if(!empty($beneficiary_service->guide_url))
                        <a href="{{ $beneficiary_service->guide_url }}" target="_blank" style="display:inline-flex; align-items:center; justify-content:center; gap:8px; padding:12px 16px; border-radius:12px; background:#fff; color:{{ $buttonColor }}; border:1px solid {{ $buttonColor }}; text-decoration:none; font-weight:700;">
                            <i class="fas fa-circle-play"></i>
                            شرح التقديم
                        </a>
                    @endif

                    @if(!empty($beneficiary_service->application_url))
                        <a href="{{ $beneficiary_service->application_url }}" target="_blank" style="display:inline-flex; align-items:center; justify-content:center; gap:8px; padding:12px 16px; border-radius:12px; background:{{ $buttonColor }}; color:#fff; border:1px solid {{ $buttonColor }}; text-decoration:none; font-weight:700;">
